...
try {
    ...
    {
        // dd("report_break_point",$request->all());
   try {
            $report = CertiIBReport::findOrFail($id);
            $certi_ib = CertiIb::findOrFail($report->app_certi_ib_id);
            $tb = new CertiIBReport;
   ...
   public function certificate_detail($token = null)
   {
       ...
       if(!empty($certi_ib_primary->certi_ib_export_mapreq_to)){
            ...
            if(!empty($certi_ib_mapreq->app_certi_ib_to)){
                ...
                $ssoUser = DB::table('sso_users')->where('username', $certi_ib_primary->tax_id)->first();  
            
                $ibHtmlTemplate = IbHtmlTemplate::where('user_id',$ssoUser->id)
                        ->where('type_standard',$certi_ib_primary->type_standard)
                        // ->where('app_certi_ib_id',$certi_ib->id)
                        ->where('type_unit',$certi_ib_primary->type_unit)
                        ->first();

                $certiIbExportMapreq = CertiIbExportMapreq::where('app_certi_ib_id',$certi_ib_primary->id)->first();
                if($certiIbExportMapreq != null)
                {
                    $cerficateIBExport = CertiIBExport::find($certiIbExportMapreq->certificate_exports_id);
                }else{
                    $cerficateIBExport =  null;
                }
                $certificateNo = ""; 
                $accreditationNo = "";
                if($cerficateIBExport != null){
                        $certificateNo = $cerficateIBExport->certificate; 
                        $accreditationNo = $cerficateIBExport->accereditatio_no;
                }
                ...
                $this->exportIbScopePdf($certi_ib_primary->id,$ibHtmlTemplate);

                    $json = $this->copyScopeIbFromAttachement($certi_ib_primary->id);
                    $copiedScopes = json_decode($json, true);

                    CertiIBFileAll::where('app_certi_ib_id',$certi_ib_primary->id)
