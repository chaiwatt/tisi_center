        ...
        $certificate_exports = CertificateExport::LeftJoin((new CertiLab)->getTable()." AS certi_labs", 'certi_labs.id', '=', 'certificate_exports.certificate_for')
            ...
            $app_certi_tracking = Tracking::LeftJoin((new CertificateExport)->getTable()." AS certificate_exports", 'certificate_exports.id', '=', 'app_certi_tracking.ref_id')
                                            ...
                                            ->where( function($query){
                                                ...
                                                ->Where('app_certi_tracking.ref_table',(new CertificateExport)->getTable());
                                            })
                                            ->where( function($query)  use($userLogIn, $roles, $app_certi_lab_id ) {
                                                if( in_array( 22 , $roles ) && Auth::check() && in_array( $userLogIn->IsGetIdRoles() , ['false'] )   ){ //ไม่ใช่ admin , ผอ , ลท ที่มี Role 22
                                                    $scope_ids =  CertificateExport::leftJoin( (new CertifyTestScope)->getTable()." AS test_scope", 'test_scope.app_certi_lab_id','certificate_exports.certificate_for')
  ...
  public function storeFilePayinDemo($setting_payment, $app_no = '', $auditor_id = '', $table_name = '', $ref_id = '', $section = '',$attach_text  = '')
  {
    ...
    public function apiRequestEditScopeFromTracking(Request $request)
    {
        ...
        if(TrackingAssessment::where('reference_refno',$appId)->first() != null)
        {
            ...
            {
                $certiLab = TrackingAssessment::where('reference_refno',$appId)->first()->certificate_export_to->applications;
                CertiLab::find($certiLab->id)->update([
                    'require_scope_update' => "1"
                ]);

                $app = CertiLab::find($certiLab->id);
                ...
                if(!is_null($app->email)){
                    $data_app =  [
                                    'certi_lab'     => TrackingAssessment::where('reference_refno',$appId)->first()->certificate_export_to->applications,
                    ...
                    $html = new  RequestEditScopeFromTracking($data_app);
...
public function update_append(Request $request ,$id)
{
  ...
  // try { 
        ...
        // dd($uploadedFile,$this->attach_path.'/'.$tracking->reference_refno);
        ...
        //    dd($attach_pdf,$tracking->certificate_export_to->certificate_for,CertiLab::where('id',$tracking->certificate_export_to->certificate_for)->first(),$tracking->FileAttachPDFTo);
    ...
    public function check_pay_in(Request $request)
    {
       ...
       if($payin == '1'){ // pay in ครั้งที่ 1
             ...
             if(!is_null($pay_in)){ 
                ...
                // dd($setting_payment);
                 $data_ref1 = $pay_in->reference_refno.'-'.$pay_in->auditors_id;

                //  dd($data_ref1);

                //  $url    =  "$setting_payment->data?pid=$setting_payment->pid&out=json&Ref1=$pay_in->reference_refno-$pay_in->auditors_id";
                 $url    =  "$setting_payment->data?pid=$setting_payment->pid&out=json&Ref1=$data_ref1";
