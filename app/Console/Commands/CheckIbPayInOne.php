...
class CheckIbPayInOne extends Command
{
    ...
    {
        Log::info('เริ่มต้นการทำงาน check:ib-payin-one-ib');
        // type = 2 ใบเสร็จ
        ...
        Log::info('พบ Transaction PayIn1 (IB) ที่ต้องตรวจสอบจำนวน: ' . $transactionPayIns->count() . ' รายการ');
            ...
            if ($result instanceof \Illuminate\Http\JsonResponse) {
                ...
                if (!empty($resultArray['message']) && $resultArray['message'] === true) {
                    ...
                    if (is_array($response) && count($response) > 0) {
                        ...
                        if ($certiIBPayInOne && $certiIBPayInOne->state != 3) {
                            Log::info('อัปเดตสถานะสำหรับ CertiIBPayInOne ID: ' . $payInOneId);
                            ...
                            $auditor = CertiIBAuditors::find($certiIBPayInOne->auditors_id);
                            if(!is_null($auditor)){
                                Log::info('อัปเดต step_id สำหรับ Auditor ID: ' . $certiIBPayInOne->auditors_id);
                                $auditor->step_id = 6; 
                                $auditor->save();
                            } else {
                                Log::warning('ไม่พบ Auditor ID: ' . $certiIBPayInOne->auditors_id . ' สำหรับ CertiIBPayInOne ID: ' . $payInOneId);
                            }
                        } else {
                             if (!$certiIBPayInOne) {
                                Log::warning('ไม่พบ CertiIBPayInOne ID: ' . $payInOneId);
                            } else {
                                Log::info('CertiIBPayInOne ID: ' . $payInOneId . ' มี state เป็น 3 อยู่แล้ว ไม่มีการอัปเดต');
                            }
                        }
        ...
        Log::info('สิ้นสุดการทำงาน check:ib-payin-one-ib');
    }
}
