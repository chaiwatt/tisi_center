...
class GenerateCbFinalReportProcessOnePdf implements ShouldQueue
{
    ...
    {
        Log::info('--- GenerateCbCbFinalReportProcessOnePdf STARTED. ---', ['file' => $this->fullFileName]);
        ...
        try {
            ...
            if ($process->isSuccessful() && Storage::disk($diskName)->exists($this->fullFileName)) {
                ...
                if (Storage::disk('ftp')->exists($this->attachPath . $this->fullFileName)) {
                    ...
                    $cerCb = $certiCBSaveAssessment->CertiCBCostTo;
                    ...
                    if ($certiCBSaveAssessment) {
                        ...
                        Log::info('http://127.0.0.1:8081/certify/check/file_cb_client/'.$storePath.'/'. $this->fullFileName);
                        Log::info('Database records created successfully.');

                    } else {
                         Log::warning('CertiCBSaveAssessment not found.', ['app_certi_cb_id' => $this->certi_cb_id]);
                    }
            ...
            Log::error('Exception in GenerateCbCbFinalReportProcessOnePdf: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            $this->fail($e);
        } finally {
