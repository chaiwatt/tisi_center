...
class GenerateIbFinalReportProcessOnePdf implements ShouldQueue
{
    ...
    {
        Log::info('--- GenerateIbFinalReportProcessOnePdf STARTED. ---', ['file' => $this->fullFileName]);
        ...
        try {
            ...
            if ($process->isSuccessful() && Storage::disk($diskName)->exists($this->fullFileName)) {
                ...
                if (Storage::disk('ftp')->exists($this->attachPath . $this->fullFileName)) {
                    ...
                    $certiIb = $certiIBSaveAssessment->CertiIBTo;
                    if ($certiIBSaveAssessment) {
                        ...
                        Log::info('http://127.0.0.1:8081/certify/check/file_ib_client/'.$storePath.'/'. $this->fullFileName);
            ...
            Log::error('Exception in GenerateIbFinalReportProcessOnePdf: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            $this->fail($e);
        } finally {
