...
class CreateTrackingLabAssessmentReportOnePdf
{
    ...
    public function generateLabAssessmentReportPdf()
    {
       ...
       $labReportOne = TrackingLabReportOne::with('attachments')->where('tracking_assessment_id',$this->assessmentId)->first();
        // dd($assessment);
      ...
      foreach ($auditors_statuses as $auditors_status)
      {
        ...
        $signer->signer_1 = SignAssessmentTrackingReportTransaction::where('tracking_report_info_id',$labReportOne->id)->where('signer_order','1')
                                                        ->where('certificate_type',2)
                                                        ->where('report_type',1)
                                                        ->first();

        
        $signer->signer_2 = SignAssessmentTrackingReportTransaction::where('tracking_report_info_id',$labReportOne->id)->where('signer_order','2')
                                                        ->where('certificate_type',2)
                                                        ->where('report_type',1)
                                                        ->first();
        $signer->signer_3 = SignAssessmentTrackingReportTransaction::where('tracking_report_info_id',$labReportOne->id)->where('signer_order','3')
                                                        ->where('certificate_type',2)
                                                        ->where('report_type',1)
                                                        ->first();
        ...
        $attachFiles = AttachFile::where('ref_table','tracking_lab_report_ones')->where('ref_id',$labReportOne->id)->where('section','11111')->get();

        // dd($attachFiles);

        $body = view('certificate.labs.assessment-labs.report-pdf.report-one.body', [
            'data' => $data,
        ...
        $footer = view('certificate.labs.assessment-labs.report-pdf.report-one.footer', []);
