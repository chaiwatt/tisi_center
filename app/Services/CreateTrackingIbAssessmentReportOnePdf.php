...
class CreateTrackingIbAssessmentReportOnePdf
{
    ...
    public function generateLabAssessmentReportPdf()
    {
       ...
       $ibReportOne = TrackingIbReportOne::with('attachments')->where('tracking_assessment_id',$this->assessmentId)->first();
    ...
    //   dd($statusAuditorMap);
        ...
        $signer->signer_1 = SignAssessmentTrackingReportTransaction::where('tracking_report_info_id',$ibReportOne->id)->where('signer_order','1')
                                                        ->where('certificate_type',1)
                                                        ->where('report_type',1)
                                                        ->first();

        
        $signer->signer_2 = SignAssessmentTrackingReportTransaction::where('tracking_report_info_id',$ibReportOne->id)->where('signer_order','2')
                                                        ->where('certificate_type',1)
                                                        ->where('report_type',1)
                                                        ->first();
        $signer->signer_3 = SignAssessmentTrackingReportTransaction::where('tracking_report_info_id',$ibReportOne->id)->where('signer_order','3')
                                                        ->where('certificate_type',1)
                                                        ->where('report_type',1)
                                                        ->first();
        ...
        $attachFiles = AttachFile::where('ref_table','tracking_ib_report_ones')->where('ref_id',$ibReportOne->id)->where('section','11111')->get();


        $body = view('certificate.ib.assessment-ib.report-pdf.report-one.body', [
            'data' => $data,
        ...
        $footer = view('certificate.ib.assessment-ib.report-pdf.report-one.footer', []);
