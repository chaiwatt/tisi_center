...
class CreateIbMessageRecordPdf
{
    ...
    public function ia($mpdf)
    {
        ...
        $boardAuditorDate = CertiIBAuditorsDate::where('auditors_id',$this->board_auditor_id)->first();
        $dateRange = "";
...
HTML;
        ...
        $body = view('certify.ib.auditor_ib.ia_ib_message_record_pdf.body', [
            'data' => $data,
        ...
        $footer = view('certify.ib.auditor_ib.ia_ib_message_record_pdf.footer', []);
        ...
        if (Storage::disk('ftp')->exists($filePath)) 
        {
            ...
            $certi_ib_attach_more->app_certi_ib_id = $boardAuditor->CertiIbCostTo->id ?? null;
            $certi_ib_attach_more->ref_id = $boardAuditor->id;
            $certi_ib_attach_more->table_name = $tb->getTable();
            $certi_ib_attach_more->file_section =  1;
            $certi_ib_attach_more->file = $storePath;
            $certi_ib_attach_more->file_client_name = 'memorandum' . '_' . $no . '.pdf';
