...
class CreateTrackingCbAssessmentReportTwoPdf
{
    ...
    public function generateLabAssessmentReportPdf()
    {
       ...
       $cbReportTwo = TrackingCbReportTwo::with('attachments')->where('tracking_assessment_id',$this->assessmentId)->first();
    ...
    //   dd($statusAuditorMap);
        ...
        $signer->signer_1 = SignAssessmentTrackingReportTransaction::where('tracking_report_info_id',$cbReportTwo->id)->where('signer_order','1')
                                                        ->where('certificate_type',0)
                                                        ->where('report_type',2)
                                                        ->first();

        
        $signer->signer_2 = SignAssessmentTrackingReportTransaction::where('tracking_report_info_id',$cbReportTwo->id)->where('signer_order','2')
                                                        ->where('certificate_type',0)
                                                        ->where('report_type',2)
                                                        ->first();
        $signer->signer_3 = SignAssessmentTrackingReportTransaction::where('tracking_report_info_id',$cbReportTwo->id)->where('signer_order','3')
                                                        ->where('certificate_type',0)
                                                        ->where('report_type',2)
                                                        ->first();
        ...
        $attachFiles = AttachFile::where('ref_table','tracking_cb_report_twos')->where('ref_id',$cbReportTwo->id)->where('section','11111')->get();


        $body = view('certificate.cb.assessment-cb.report-pdf.report-two.body', [
            'data' => $data,
        ...
        $footer = view('certificate.cb.assessment-cb.report-pdf.report-two.footer', []);
