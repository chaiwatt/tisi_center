...
<head>
    ...
    <title>Document Editor หนังสือแต่งตั้งตรวจเอกสาร CB</title>
...
<body>
    ...
    <script>
        ...
        const signedAll = @json($signedAll ?? null);
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function downloadTemplate()
            {
                ...
                fetch("{{ route('cb.download-assessment-review-html-template') }}", {
                    method: 'POST',
                    headers: {
            ...
            loadDefaultTemplateBtn.addEventListener('click', () => {
                ...
                fetch("{{ route('cb.default-download-assessment-review-html-template') }}", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            ...
            function saveData(status) {
                ...
                fetch("{{ route('cb.save-assessment-review-html-template') }}", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
