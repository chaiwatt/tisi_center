...
<div class="row">
     <div class="col-md-12">
        <div class="col-md-9">
          {{-- <div class="form-group {{ $errors->has('reference_refno') ? 'has-error' : ''}}">
           ...
           <div class="form-group {{ $errors->has('reference_refno') ? 'has-error' : '' }}">
            ...
            <div class="col-md-7">
                @if (!empty($tracking->reference_refno))
                    <input type="text" name="reference_refno" id="reference_refno" class="form-control" value="{{ $tracking->reference_refno }}" disabled>
                    <input type="hidden" name="tracking_id" id="tracking_id" class="form-control" value="{{ $tracking->tracking_id ?? '' }}">
                @else
                    <input type="text" name="reference_refno" id="reference_refno" class="form-control no-drop" value="" readonly>
                @endif
                <input type="hidden" name="certificate_for" id="certificate_for" class="form-control" value="{{ $tracking->certificate_for ?? '' }}" disabled>
            </div>
        </div>
        
        <div class="form-group {{ $errors->has('org_name') ? 'has-error' : '' }}">
            <label for="org_name" class="col-md-5 control-label">ชื่อผู้ยื่นคำขอ :</label>
            <div class="col-md-7">
                <input type="text" name="no" id="org_name" class="form-control no-drop" value="{{ $tracking->name ?? '' }}" readonly>
            </div>
        </div>
        
        <div class="form-group {{ $errors->has('lab_name') ? 'has-error' : '' }}">
            <label for="lab_name" class="col-md-5 control-label">ชื่อห้องปฏิบัติการ :</label>
            <div class="col-md-7">
                <input type="text" name="lab_name" id="lab_name" class="form-control" value="{{ $tracking->lab_name ?? '' }}" disabled>
            </div>
        </div>
        ...
        <div class="form-group {{ $errors->has('auditor') ? 'has-error' : '' }}">
            ...
            <div class="col-md-7">
                <input type="text" name="auditor" id="auditor" class="form-control" value="{{ old('auditor') }}" maxlength="255" required>
            ...
            <div class="form-group {{ $errors->has('other_attach') ? 'has-error' : ''}}">
                ...
                <div class="col-md-7">
                   @if (isset($auditor))
                    @if (!empty($auditor->FileAuditorsLatestFile()) )
                        @php
                            $memorandumFile = $auditor->FileAuditorsLatestFile();
                        @endphp
                        <p id="deleteFlieOtherAttach">
                            <a href="{{url('funtions/get-view/'.$memorandumFile->url.'/'.( !empty($memorandumFile->filename) ? $memorandumFile->filename :  basename($memorandumFile->new_filename) ))}}" target="_blank">
                                {!! HP::FileExtension($memorandumFile->filename)  ?? '' !!}
                            </a>
                            <button class="btn btn-danger btn-xs deleteFlie   {{ (!empty($auditor) && ($auditor->vehicle == 1 || $auditor->status_cancel == 1)) ? 'hide' : ''}}" type="button" onclick="deleteFlieOtherAttach({{ $memorandumFile->id}})">
                    ...
                    @endif
                    @else
                    <span class="text-warning">ยังไม่ได้แต่งตั้ง</span>
                   @endif
            ...
            <div class="form-group"  hidden>
                ...
                <div class="form-group">
                    <label for="signer_2" class="col-md-5 control-label">
                        <span class="text-danger">*</span> ผู้ลงนาม (ผก.)
                    </label>
                    <div class="col-md-7">
                ...
                <div class="form-group">
                    <label for="signer_3" class="col-md-5 control-label">
                        <span class="text-danger">*</span> ผู้ลงนาม (ผอ. สก.)
                    </label>
                    <div class="col-md-7">
                ...
                <div class="form-group">
                    <label for="signer_4" class="col-md-5 control-label">
                        <span class="text-danger">*</span> ผู้ลงนาม (ลมอ. / ผอ. สก.)
                    </label>
                    <div class="col-md-7">
                ...
                <div class="col-md-7">
                    @if (isset($auditor))
                         @if ($auditor->message_record_status == 1)
                            <a class="button btn btn-warning" href="{{route('certificate.auditor-labs.create-tracking-lab-message-record',['id' => $auditor->id])}}"><i class="fa fa-book" aria-hidden="true"> </i></a>
                             @elseif($auditor->message_record_status == 2)
                             <a class="button btn btn-info" href="{{route('certificate.auditor-labs.view-tracking-lab-message-record',['id' => $auditor->id])}}"><i class="fa fa-book" aria-hidden="true"> </i></a>
                         @endif
...
<div class="col-md-12 repeater">
    ...
    <table class="table color-bordered-table primary-bordered-table">
        <thead>
        <tr>
            <th class="text-center" style="width:20%">สถานะผู้ตรวจประเมิน</th>
            <th class="text-center" style="width:20%">ชื่อผู้ตรวจประเมิน</th>
      ...
      @if (!empty($auditors_status))
        ...
        <tr class="repeater-item">
            <td class="text-center text-top" style="width: 30%">
                <div class="form-group {{ $errors->has('taxid') ? 'has-error' : ''}}">
                    ...
                    <div class="col-md-9">
                        ...
                        <select name="list[status][]" 
                                ...
                                data-key="{{ $key }}" 
                                required>
                            <option value="" disabled selected>-เลือกสถานะผู้ตรวจประเมิน-</option>
                                @foreach(App\Models\Bcertify\StatusAuditor::pluck('title', 'id') as $id => $title)
                                    <option value="{{ $id }}" {{ (isset($item->status_id) && $item->status_id == $id) ? 'selected' : '' }}>
                                        {{ $title }}
                                    </option>
                                @endforeach
                        </select>
...
@push('js')
  ...
  <script type="text/javascript">
          ...
          $(document).ready(function () {
            ...
            //         // this หมายถึง tr ปัจจุบันใน loop
            //         const row = $(this);
                    
            //         // ดึงค่าจาก td ตาม index
            //         const value1 = row.find('td').eq(0).text(); // ค่าของ td ลำดับที่ 0
            //         const value2 = row.find('td').eq(1).text(); // ค่าของ td ลำดับที่ 1
            //         const value3 = row.find('td').eq(2).text(); // ค่าของ td ลำดับที่ 2
            //         const value4 = row.find('td').eq(3).text(); // ค่าของ td ลำดับที่ 3

            //         // ตัวอย่าง: แสดงผลค่าที่ดึงได้
            //         console.log(`Row values: ${value1}, ${value2}, ${value3}, ${value4}`);
            //     });
            //     // return;
            ...
            $('#form_auditor').parsley().on('field:validated', function () {
                ...
                // กำหนดค่า user_id ก่อนส่งฟอร์ม
                $('#signaturesJson').val(20);
     ...
     function  deleteFlieOtherAttach(id,$attachs){
        ...
        //     // ตรวจสอบฟอร์มด้วย Parsley
        //     if ($('#form_auditor').parsley().isValid()) {
        //         Swal.fire({
