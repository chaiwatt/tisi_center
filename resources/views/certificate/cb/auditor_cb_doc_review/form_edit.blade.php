...
<div class="row">
     <div class="col-md-12">
        ...
        <div class="form-group">
            ...
            <label  class="col-md-7 control-label" style="text-align: left">
                <span class="text-danger"> </span> {{$tracking->certificate_export_to->applications->name_standard}}
            </label>
...
@push('js')
 ...
 var certiCb = null;
  ...
  let trackingId = $('#tracking_id').val();
        ...
        $.ajax({
            url: "{{route('tracking.cb_cancel_doc_review_team')}}",
            method: "POST",
            data: {
    ...
    $('#save_doc_review').click(function(){
        ...
        if(agreeValue == 1)
        {
            ...
            $.ajax({
                url: "{{route('tracking.cb_reject_doc_review')}}",
                method: "POST",
                data: {
                    trackingId: trackingId,
        ...
        {
            $.ajax({
                url: "{{route('tracking.cb_accept_doc_review')}}",
                method: "POST",
                data: {
                    trackingId: trackingId,
                    _token: _token
