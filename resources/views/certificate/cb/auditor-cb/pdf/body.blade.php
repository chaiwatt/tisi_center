...
<table style="margin-top: 10px;margin-bottom: 5px">
    <tr style="display:block">
        ...
        <td style="border-bottom: 1px dotted #000; padding-bottom: 1;line-height: 0.6;width: 600px;padding-left:20px">
            การแต่งตั้งคณะผู้ตรวจประเมินหน่วยตรวจ {{$data->name_unit}} ( คำขอเลขที่ {{$tracking->reference_refno}} )
        </td>
    </tr>
</table>
...
<div style="position: relative;margin-top:20px;line-height:1.15">
    ...
    <div>
        @php
            $textResult = TextHelper::callLonganTokenizePost("วันที่!" . $data->register_date . "!".str_replace(' ', '!', $data->name_unit)." ได้ยื่นคำขอรับใบรับรองหน่วยตรวจ ในระบบ E-Accreditation และสามารถรับคำขอได้เมื่อวันที่ ". $data->get_date);
...
<div style="text-align: center;">
    ...
    <div style="position: relative;padding-top:10px; line-height:1.15">
        ...
        @php
            $textResult = TextHelper::callLonganTokenizePost("รต.".$boardAuditorMsRecordInfo->body_text2." รต. ได้สรรหาคณะผู้ตรวจประเมินประกอบด้วย หัวหน้าผู้ตรวจประเมิน ผู้ตรวจประเมินและผู้เชี่ยวชาญ เพื่อดำเนินการตรวจประเมินสถานประกอบการ".str_replace(' ', '!', $data->name_unit). " ". str_replace(' ', '!', $data->date_range). " ดังนี้");
                        ...
                        @foreach ($auditors as $auditor)
                            @foreach ($auditor->auditors_status_many as $auditorStatus)
                                @foreach ($auditorStatus->auditors_list_many as $key => $item)
                                    <tr>
                                        <td style="width: 180px">({{$key+1}}). {{$item->temp_users}}</td>
                                        <td style="width: 180px">{{$item->temp_departments}}</td>
                                    </tr>
                                @endforeach
                            @endforeach
                        @endforeach
        ...
        <div style="text-indent: 100px;display:block;font-size:22px;">จึงเรียนมาเพื่อโปรดพิจารณา หากเห็นเป็นการสมควร ขอได้โปรดนำเรียน ลมอ. เพื่ออนุมัติ การแต่งตั้งคณะผู้ตรวจประเมินสถานประกอบการ{{$data->name_unit}} ในวันที่ {{$data->date_range}} รายละเอียด ดังข้างต้น</div>
    </div>
...
<div style="padding-top:30px">
    <table style="width: 100%;text-align:center;">
        ...
        <div style="position: relative;padding-top:10px ;line-height:1.18">
            ...
            <div style="letter-spacing: 0.1px;">การรับรองหน่วยตรวจ {{$data->name_unit}} ตามวันและเวลา ดังกล่าว</div>
