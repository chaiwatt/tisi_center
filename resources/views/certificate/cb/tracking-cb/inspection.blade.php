...
<div class="modal fade" id="modal-request-edit-scope">
 ...
 {!! Form::open(['url' => 'certificate/tracking-cb/update_inspection/'.$inspection->id,
...
<div class="row form-group">
 ...
 <div class="row form-group">
  <div class="  {{ $errors->has('cb_name') ? 'has-error' : ''}}">
      {!! HTML::decode(Form::label('cb_name', 'ชื่อผู้ยื่นคำขอ :', ['class' => 'col-md-3 control-label text-right'])) !!}
      <div class="col-md-8 ">
          {!! Form::text('cb_name', (!empty($tracking->certificate_export_to->CertiCbTo->name) ? $tracking->certificate_export_to->CertiCbTo->name  : null) , ['id' => 'cb_name', 'class' => 'form-control',  'disabled' => true]); !!}
      </div>
  </div>
</div>
 <div class="row form-group">
  <div class="  {{ $errors->has('name_standard') ? 'has-error' : ''}}">
      {!! HTML::decode(Form::label('name_standard', 'ชื่อหน่วยรับรอง :', ['class' => 'col-md-3 control-label text-right'])) !!}
      <div class="col-md-8 ">
          {!! Form::text('name_standard', (!empty($tracking->certificate_export_to->CertiCbTo->name_standard) ? $tracking->certificate_export_to->CertiCbTo->name_standard  : null) , ['id' => 'name_standard', 'class' => 'form-control',  'disabled' => true]); !!}
      </div>
  </div>
</div>
 <div class="row form-group"> 
                        ...
                        <div class="form-group other_attach_scope">
                            ...
                            <div class="col-md-6">
                                ...
                                @if ($inspection->certiCb() == null)
                                    ...
                                    @else
                                        @if ($inspection->certiCb()->require_scope_update == null)
                                            <a type="button" class="btn btn-sm btn-info attach-add" id="button_show_request_edit_scope_modal">
                                                <i class="fa fa-pencil-square-o"></i>&nbsp;ขอให้แก้ไข
                                            </a>
                                          @else  
...
<script src="{{ asset('plugins/components/icheck/icheck.init.js') }}"></script>
    ...
    <script>
        ...
        $(document).on('click', '#button_request_edit_scope', function(e) {
            ...
            // เรียก AJAX
            $.ajax({
                url: "{{route('api_request_edit_cb_scope_from_tracking')}}",
                method: "POST",
                data: {
                    _token: _token,
