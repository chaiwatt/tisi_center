...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ระบบตรวจติดตามใบรับรองระบบหน่วยรับรอง (CB)</h3>

                    <div class="pull-right">

                      @if(isset($select_users) && count($select_users) > 0) 
                      @can('edit-'.str_slug('trackingcb'))

                        @can('follow_up_before-'.str_slug('trackingcb'))
                            <button type="button" class="btn btn-success"  id="button_check"> ตรวจติดตามก่อนกำหนด  </button>
                        @endcan
                        @can('receive_inspection-'.str_slug('trackingcb'))
                            <button type="button" class="btn btn-info"  id="button_receiver"> รับเรื่องตรวจติดตาม  </button>
                        @endcan
                        @can('assign_work-'.str_slug('trackingcb'))
                          ...
                          <div class="modal-dialog modal-xl" role="document">
                              <div class="modal-content">
                                  <div class="modal-header">
                                      ...
                                      <h4 class="modal-title" id="exampleModalLabel1">ระบบตรวจติดตามใบรับรองระบบหน่วยรับรอง (CB)</h4>
                                  </div>
                    ...
                    <div class="row ">
                      ...
                      <div class="col-md-2">
                              {!! Form::select('filter_certificate_no',
                                    App\Models\Certify\ApplicantCB\CertiCBExport::whereIn('status',[3,28])->pluck('certificate','id'), 
                                null, 
                                ['class' => 'form-control', 
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <tr>
                              ...
                              <th width="10%" class="text-center">ชื่อหน่วยรับรอง</th>
...
<div class="modal-body">
    ...
    @include('certificate/cb/tracking-cb/modal.check')
    @include('certificate/cb/tracking-cb/modal.receiver')
@endsection
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.modal_status', function(){
                    ...
                    $.ajax({
                                type:"GET",
                                url:  "{{ url('/certificate/tracking-cb/modal_status_auditor') }}",
                                data:{
                                    _token: "{{ csrf_token() }}",
                                    id: id,
                                    refno: refno
                  ...
                  if(id.length > 0 && checker.length > 0){
                       // Text
                        $.LoadingOverlay("show", {
                          ...
                          $.ajax({
                                  type:"GET",
                                  url:  "{{ url('/certificate/tracking-cb/assign_cb') }}",
                                  data:{
                                      _token: "{{ csrf_token() }}",
                                      ids: id,
                                      checker: checker
