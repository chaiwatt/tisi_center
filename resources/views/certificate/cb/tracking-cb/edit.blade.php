...
<div class="container-fluid" >
    ...
    <div class="row">
        <div class="col-sm-12">
                <h2 class=" pull-left" style="color:Black;">ระบบตรวจติดตามใบรับรองระบบหน่วยรับรอง (CB) {!! !empty($cer->certificate)? ' : '.$cer->certificate :''  !!} </h2>
                @can('view-'.str_slug('trackingcb'))
                    <a class="btn btn-danger  pull-right" href="{{ url('/certificate/tracking-cb') }}">
...
<a class="btn btn-info form_group"   href="{{ url('certificate/tracking-cb').'/'.$cer->app_certi_cb_id }}" >
    ข้อมูลใบรับรอง
</a>
<!-- end  ข้อมูลใบรับรอง -->
@php 
...
{{-- @if($user->IsGetIdLathRoles() == 'false'  || $user->IsGetRolesAdmin() == 'true') --}}
 ...
 @endphp
   <div class="btn-group form_group">
      <div class="btn-group">
             {{-- <a  class="btn  {{$auditors_btn}}" href="{{ url("certificate/auditor-cbs")}}" target="_blank">
             ...
             <div class="dropdown-menu" role="menu" >
               ...
               @foreach($tracking->AuditorsManyBy as $key => $item)
                ...
                @if ($item->status_cancel != 1)
                    <a  class="btn {{$auditors_btn}} " href="{{ url("certificate/auditor-cbs/".$item->id."/edit")}}" style="background-color:{{$auditors_btn}};width:750px;text-align: left">
                        ครั้งที่ {{ ($i_key + 1 )}} :  
                        {{ $item->auditor ?? '-'}}
                    </a> 
                    <br>
...
{{-- @endif      --}}
              ...
              @php $key_payin_one = 0;   @endphp
               @foreach($tracking->tracking_payin_one_many as $key => $item)
                     @php 
                      ...
                      @endphp
                      @if ($item->status   != 3) 
                          <a  class="btn {{$payin1_btn}} " href="{{ url("certificate/tracking-cb/Pay_In1/".$item->id)}}" style="width:750px;text-align: left">
                              ครั้งที่ {{  ($key_payin_one +1) }} :      {{  (!empty($item->auditors_to->auditor) ?  $item->auditors_to->auditor : '') }}
 ...
 @endphp
  <div class="btn-group form_group">
      <div class="btn-group">
                 {{-- <a  class="btn {{$assessment_btn}}" href="{{ url("certificate/assessment-cb")}}" target="_blank">
 ...
 @php
          ...
          if ($assessment->degree == 7 || $assessment->degree == 4) { // ผ่านการการประเมิน
              $assessment_btn =  'btn-info';
              $assessment_url =  'certificate/assessment-cb/'.$assessment->id.'/edit';
 ...
 @endforeach
  ...
  @endphp
        <a  class="form_group btn {{$inspection_btn}}" href="{{ url("certificate/inspection-cb/$inspection->id")}}" >
  ...
  @endif 
    ...
    @endphp
        ...
        <a  class="form_group btn {{$pay_in2_btn}}" href="{{ url("certificate/tracking-cb/pay-in2/$pay_in2->id")}}" >
...
{{-- 
    @if( $tracking->status_id >= 6  && !is_null($tracking->tracking_report_to) )
        @include('certificate.cb.tracking-cb.modal.report',['certi'=> $tracking,'report' => $tracking->tracking_report_to ])   
    @endif 
     --}}
    @if( $tracking->status_id >= 6  && !is_null($tracking->tracking_review_to) )    
        @include('certificate.cb.tracking-cb.modal.review',['certi'=> $tracking,'review' => $tracking->tracking_review_to])
    @endif 
...
<div class="white-box">
    <div class="row">
        <div class="col-sm-12">
            <h3>สถานะการตรวจติดตามใบรับรองหน่วยรับรอง</h3>
            <hr>
<div class="row">
    <div class="col-sm-12">
...
<div class="white-box">
    <div class="row">
        <div class="col-sm-12">
            <h3>ประวัติคำขอรับใบรับรองหน่วยรับรอง </h3>
            <hr>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @if($item->DataSystem != '-')
                                    ...
                                    @include ('certificate.cb.tracking-cb.history_detail',['history' => $item])
                                @else 
                                    -
                                @endif
