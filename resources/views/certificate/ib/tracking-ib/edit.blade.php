...
<div class="container-fluid" >
    ...
    <div class="row">
        <div class="col-sm-12">
                <h2 class=" pull-left" style="color:Black;">ระบบตรวจติดตามใบรับรองระบบหน่วยตรวจ (IB) {!! !empty($cer->certificate)? ' : '.$cer->certificate :''  !!} </h2>
                @can('view-'.str_slug('trackingib'))
                    <a class="btn btn-danger  pull-right" href="{{ url('/certificate/tracking-ib') }}">
...
<a class="btn btn-info form_group"   href="{{ url('certificate/tracking-ib').'/'.$cer->app_certi_ib_id }}" >
    ข้อมูลใบรับรอง
</a>
<!-- end  ข้อมูลใบรับรอง -->
@php 
             ...
             <div class="dropdown-menu" role="menu" >
               ...
               @foreach($tracking->AuditorsManyBy as $key => $item)
                ...
                {{-- @if ($item->status_cancel != 1) --}}
                    <a  class="btn {{$auditors_btn}} " href="{{ url("certificate/auditor-ibs/".$item->id."/edit")}}" style="background-color:{{$auditors_btn}};width:750px;text-align: left">
                        ครั้งที่ {{ ($i_key + 1 )}} :  
                        {{ $item->auditor ?? '-'}}
                    </a> 
                    <br>
...
{{-- @endif      --}}
   ...
   <!-- START  admin , ผอ , ผก , เจ้าหน้าที่ ลท.padding-top: 100px -->
             ...
             <div class="dropdown-menu" role="menu" >
               ...
               @foreach($tracking->tracking_payin_one_many as $key => $item)
                     @php 
                      ...
                      @endphp
                      @if ($item->status   != 3) 
                          <a  class="btn {{$payin1_btn}} " href="{{ url("certificate/tracking-ib/Pay_In1/".$item->id)}}" style="width:750px;text-align: left">
 ...
 {{-- @if((auth()->user()->SetRolesLicenseCertify() == "true" ||  in_array("29",auth()->user()->RoleListId)) && count($cer->tracking_payin_one_status1_many)  > 0 )   --}}
 ...
 @php
          ...
          if ($assessment->degree == 7 || $assessment->degree == 4) { // ผ่านการการประเมิน
              $assessment_btn =  'btn-info';
              $assessment_url =  'certificate/assessment-ib/'.$assessment->id.'/edit';
 ...
 @endphp
     <a  class="btn {{$assessment_btn}}  " href="{{ url("$assessment_url")}}"  style="background-color:{{$assessment_btn}};width:750px;text-align: left">
         {{-- ครั้งที่ {{ count($tracking->tracking_assessment_many) - ($key) }} :   --}}
         ครั้งที่ {{  ($key +1) }} :  
...
@endif
  ...
  @php 
       ...
       if($tracking->status_id == 4 && is_null($inspection->status)){
         ...
         $inspection_icon =  '';
       }else if($tracking->status_id == 4 && !is_null($inspection->FileAttachScopeTo)){
  ...
  @endphp
        <a  class="form_group btn {{$inspection_btn}}" href="{{ url("certificate/inspection-ib/$inspection->id")}}" >
  ...
  @endif 
    ...
    @endphp
        ...
        <a  class="form_group btn {{$pay_in2_btn}}" href="{{ url("certificate/tracking-ib/pay-in2/$pay_in2->id")}}" >
...
{{-- {{$tracking->status_id}} --}}
 ...
 {{-- @endif  --}}
  <!-- END -->  
    ...
    {{-- @if( $tracking->status_id >= 6  && !is_null($tracking->tracking_report_to) )
        @include('certificate.ib.tracking-ib.modal.report',['certi'=> $tracking,'report' => $tracking->tracking_report_to ])   
    @endif   --}}
    
    @if( $tracking->status_id >= 6  && !is_null($tracking->tracking_review_to) )    
        @include('certificate.ib.tracking-ib.modal.review',['certi'=> $tracking,'review' => $tracking->tracking_review_to])
    @endif 
...
<div class="white-box">
    <div class="row">
        <div class="col-sm-12">
            <h3>สถานะการตรวจติดตามใบรับรองหน่วยตรวจ</h3>
            <hr>
<div class="row">
    <div class="col-sm-12">
    ...
    <div class="col-sm-12">
        <p class="col-md-3 text-right">ชื่อหน่วยตรวจสอบ : </p>
        <p class="col-md-9"> {!! !empty($tracking->certificate_export_to->CertiIBCostTo->name_unit)?  $tracking->certificate_export_to->CertiIBCostTo->name_unit:''  !!} </p>
    </div>
    <div class="col-sm-12">
...
<div class="white-box">
    <div class="row">
        <div class="col-sm-12">
            <h3>ประวัติคำขอรับใบรับรองหน่วยตรวจ </h3>
            <hr>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @if($item->DataSystem != '-')
                                    ...
                                    @include ('certificate.ib.tracking-ib.history_detail',['history' => $item])
                                @else 
                                    -
                                @endif
...
<script src="{{ asset('plugins/components/icheck/icheck.init.js') }}"></script>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            //     if(data_hide == 1){
            //         $('.data_hide').hide ();
            //         $('.input_readonly').prop('disabled', true);
            ...
            if(review == 1){
                     ...
                     $('.checkbox-review').parent().css({"background-color": "rgb(238, 238, 238);","cursor": "not-allowed"});
        ...
        $("#btn_doc_auditor").on("click", function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    const baseUrl = "{{ url('/certificate/auditor_ib_doc_review/auditor_ib_doc_review') }}";
