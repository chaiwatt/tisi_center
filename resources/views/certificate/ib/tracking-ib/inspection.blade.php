...
<div class="modal fade" id="modal-request-edit-scope">
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="white-box">
                    <h3 class="box-title pull-left"> ระบบสรุปผลการตรวจประเมินติดตาม </h3>
 ...
 {!! Form::open(['url' => 'certificate/tracking-ib/update_inspection/'.$inspection->id,
...
<div class="row form-group">
  ...
  <div class="  {{ $errors->has('reference_refno') ? 'has-error' : ''}}">
      ...
      <div class="col-md-8 ">
          {!! Form::text('reference_refno', (!empty($inspection->reference_refno) ? $inspection->reference_refno  : null) , ['id' => 'reference_refno', 'class' => 'form-control',  'disabled' => true]); !!}
      </div>
  </div>
</div>
 <div class="row form-group">
  <div class="  {{ $errors->has('org_name') ? 'has-error' : ''}}">
      ...
      <div class="col-md-8 ">
          {!! Form::text('org_name', (!empty($tracking->certificate_export_to->CertiIBCostTo->name) ? $tracking->certificate_export_to->CertiIBCostTo->name  : null) , ['id' => 'org_name', 'class' => 'form-control',  'disabled' => true]); !!}
      </div>
  </div>
</div>
 <div class="row form-group">
  <div class="  {{ $errors->has('name_unit') ? 'has-error' : ''}}">
      {!! HTML::decode(Form::label('name_unit', 'ชื่อหน่วยตรวจสอบ :', ['class' => 'col-md-3 control-label text-right'])) !!}
      <div class="col-md-8 ">
          {!! Form::text('name_unit', (!empty($tracking->certificate_export_to->CertiIBCostTo->name_unit) ? $tracking->certificate_export_to->CertiIBCostTo->name_unit  : null) , ['id' => 'name_unit', 'class' => 'form-control',  'disabled' => true]); !!}
      </div>
  </div>
</div>
 <div class="row form-group"> 
                ...
                <div class="panel block4">
                    ...
                    <div class="panel panel-info">
                        <div class="panel-heading">
                            ...
                            @endif
                            @if(!empty($item->FileAttachAssessment4Many) && count($item->FileAttachAssessment4Many) > 0) 
                            <div class="row form-group"> 
                                    <div class="  {{ $errors->has('amount_bill_all') ? 'has-error' : ''}}">
                        ...
                        <div class="form-group other_attach_scope">
                            ...
                            <div class="col-md-6">
                                ...
                                @if ($inspection->certiIb() == null)
                                    ...
                                    @else
                                        @if ($inspection->certiIb()->require_scope_update == null)
                                            <a type="button" class="btn btn-sm btn-info attach-add" id="button_show_request_edit_scope_modal">
                                                <i class="fa fa-pencil-square-o"></i>&nbsp;ขอให้แก้ไข
                                            </a>
                                          @else  
...
<script src="{{ asset('plugins/components/icheck/icheck.init.js') }}"></script>
    ...
    <script>
        ...
        $(document).on('click', '#button_request_edit_scope', function(e) {
            ...
            // เรียก AJAX
            $.ajax({
                url: "{{route('api_request_edit_ib_scope_from_tracking')}}",
                method: "POST",
                data: {
                    _token: _token,
