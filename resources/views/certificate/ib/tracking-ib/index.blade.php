...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">ระบบตรวจติดตามใบรับรองระบบหน่วยตรวจ (IB)</h3>

                    <div class="pull-right">

                      @if(isset($select_users) && count($select_users) > 0) 
                      @can('edit-'.str_slug('trackingib'))

                        @can('follow_up_before-'.str_slug('trackingib'))
                            <button type="button" class="btn btn-success"  id="button_check"> ตรวจติดตามก่อนกำหนด  </button>
                        @endcan
                        @can('receive_inspection-'.str_slug('trackingib'))
                            <button type="button" class="btn btn-info"  id="button_receiver"> รับเรื่องตรวจติดตาม  </button>
                        @endcan
                        @can('assign_work-'.str_slug('trackingib'))
                          ...
                          <div class="modal-dialog modal-xl" role="document">
                              <div class="modal-content">
                                  <div class="modal-header">
                                      ...
                                      <h4 class="modal-title" id="exampleModalLabel1">ระบบตรวจติดตามใบรับรองระบบหน่วยตรวจ (IB)</h4>
                                  </div>
                    ...
                    <div class="row ">
                      ...
                      <div class="col-md-2">
                              {!! Form::select('filter_certificate_no',
                                    App\Models\Certify\ApplicantIB\CertiIBExport::groupBy('certificate')->pluck('certificate','certificate'), 
                                null, 
                                ['class' => 'form-control', 
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <tr>
                              ...
                              <th width="10%" class="text-center">ชื่อหน่วยตรวจสอบ</th>
...
<div class="modal-body">
    ...
    @include('certificate/ib/tracking-ib/modal.check')
    @include('certificate/ib/tracking-ib/modal.receiver')
@endsection
    ...
    <script>
        $(document).ready(function () {
            ...
            var table = $('#myTable').DataTable({
                ...
                searching: false,
                ajax: {
                    url: '{!! url('/certificate/tracking-ib/data_list') !!}',
                    data: function (d) {
            ...
            $(document).on('click', '.modal_status', function(){
                    ...
                    $.ajax({
                                type:"GET",
                                url:  "{{ url('/certificate/tracking-ib/modal_status_auditor') }}",
                                data:{
                                    _token: "{{ csrf_token() }}",
                                    id: id,
                                    refno: refno
            ...
            $('#button_checker').click(function(event) {
                  var row = $('.item_checkbox:checked').length;
                  if(row > 0){
            ...
            $(document).on('click', '#submit_form', function(){
                  ...
                  if(id.length > 0 && checker.length > 0){
                          ...
                          $.ajax({
                                  type:"GET",
                                  url:  "{{ url('/certificate/tracking-ib/assign_ib') }}",
                                  data:{
                                      _token: "{{ csrf_token() }}",
                                      ids: id,
                                      checker: checker
