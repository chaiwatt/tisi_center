...
<div class="row">
     <div class="col-md-12">
        <div class="col-md-9">
          {{-- <div class="form-group {{ $errors->has('reference_refno') ? 'has-error' : ''}}">
           ...
           <div class="form-group {{ $errors->has('reference_refno') ? 'has-error' : '' }}">
            ...
            <div class="col-md-7">
                @if(!empty($certiIb->app_no))
                    <input type="text" id="reference_refno" name="reference_refno" class="form-control" value="{{ $tracking->reference_refno}}" disabled>
                    <input type="hidden" id="tracking_id" name="tracking_id" class="form-control" value="{{ $tracking->id ?? '' }}">
                @else
        ...
        <div class="form-group">
            ...
            <label  class="col-md-7 control-label" style="text-align: left">
                <span class="text-danger"> </span> {{$tracking->certificate_export_to->applications->name_unit}}
            </label>
        ...
        <div class="form-group">
            ...
            <label  class="col-md-7 control-label" style="text-align: left">
                <span class="text-danger"> </span> {{$trackingDocReviewAuditor->team_name}}
            </label>
        ...
        <div class="form-group">
            ...
            <label  class="col-md-7 control-label" style="text-align: left">
                <span class="text-danger"> </span> {{ HP::DateThai($trackingDocReviewAuditor->from_date) }} ถึง {{ HP::DateThai($trackingDocReviewAuditor->to_date) }}
            </label>
        ...
        <div class="form-group">
            ...
            <label  class="col-md-7 control-label" style="text-align: left">
                <span class="text-danger"> </span> @if ($trackingDocReviewAuditor->type == 1)
                ตรวจประเมิน ณ สถานประกอบการ
                @elseif($trackingDocReviewAuditor->type == 2)
                ตรวจประเมิน ณ สมอ.
                @endif
            </label>
...
@if ($tracking->trackingDocReviewAuditor->status == 2)
    <div class="row" style="margin-top: 20px">
        <div class="col-md-2">
        </div>
        <div class="col-md-8">
            ...
            <div class="form-group" style="padding:15px">
                <label  class="col-md-12 control-label" style="text-align: left">
                    <span class="text-danger">  {{ $tracking->trackingDocReviewAuditor->remark_text }} </span>
                </label>
            </div>
 ...
 var certiIb = null;
    ...
    $('#save_doc_review').click(function(){
        ...
        if(agreeValue == 1)
        {
            ...
            $.ajax({
                url: "{{route('tracking.ib_reject_doc_review')}}",
                method: "POST",
                data: {
                ...
                success: function(result) {
                    ...
                    // let redirectUrl = baseUrl + "/certify/check_certificate-cb/" + certiIb.token + "/show/" + certiIb.id;
                       let redirectUrl =  baseUrl + "/certificate/tracking-ib/"+trackingId+"/edit/";
        ...
        {
            $.ajax({
                url: "{{route('tracking.ib_accept_doc_review')}}",
                method: "POST",
                data: {
                    trackingId: trackingId,
                    _token: _token
  ...
  let trackingId = $('#tracking_id').val();
        ...
        $.ajax({
            url: "{{route('tracking.ib_cancel_doc_review_team')}}",
            method: "POST",
            data: {
                trackingId: trackingId,
                _token: _token
