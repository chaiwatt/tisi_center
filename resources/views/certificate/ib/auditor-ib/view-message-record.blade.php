...
<body>
    <div class="wrapper">
        ...
        <form id="ibMessageForm" method="POST" action="{{ route('certificate.auditor-ibs.save-tracking-ib-message-record') }}">
            @csrf
            <!-- ส่วนราชการ -->
            ...
            <table class="table-section">
                <tr>
                    ...
                    <td style="width: 300px;">
                        <input type="text" class="input-no-border" id="header_text1" name="header_text1" value="{{$boardAuditorMsRecordInfo->header_text1}}" readonly >
                    </td>
                    <td style="font-size:22px">โทรศัพท์</td>
                    <td style="width: 235px">
                        <input type="text" class="input-no-border" id="header_text2" name="header_text2" value="{{$boardAuditorMsRecordInfo->header_text2}}" readonly>
                    </td>
                </tr>
            </table>
            ...
            <table class="table-section">
                <tr>
                    ...
                    <td style="width: 380px;">
                        <input type="text" class="input-no-border" id="header_text3" name="header_text3" value="{{$boardAuditorMsRecordInfo->header_text3}}" readonly>
                    </td>
                    <td>วันที่</td>
                    <td style="width: 300px;">
                        <input type="text" class="input-no-border" id="header_text4" name="header_text4" value="{{$boardAuditorMsRecordInfo->header_text4}}" readonly>
                    </td>
                </tr>
            </table>
            ...
            <table class="table-section" >
                <tr>
                    <td>เรื่อง</td>
                    <td style="width: 700px;font-size:22px" class="under-line">การแต่งตั้งคณะผู้ตรวจหน่วยตรวจ {{$data->name_standard}} (คำขอเลขที่ {{$tracking->reference_refno}}) </td>
                </tr>
            </table>

            {{$boardAuditorMsRecordInfo}}
            <!-- Main Content -->
            <div class="section">
                <div>เรียน ผอ.สก. ผ่าน ผก.รต.<input type="text" class="input-no-border" id="body_text1" name="body_text1" value="{{$boardAuditorMsRecordInfo->body_text1}}" style="width:30px" readonly></div>
            ...
            <div class="section">
                ...
                <div style="text-indent: 137px;margin-top:10px;line-height:34px">
                    รต.<input type="text" class="input-no-border" id="body_text2" name="body_text2" value="{{$boardAuditorMsRecordInfo->body_text2}}" style="width:30px" required > สก. ได้สรรหาคณะผู้ตรวจประเมินประกอบด้วย หัวหน้าผู้ตรวจประเมิน ผู้ตรวจประเมินและผู้เชี่ยวชาญ
                    เพื่อดำเนินการตรวจประเมินสถานประกอบการ ของ {{$data->name_standard}} ในวันที่ {{$data->date_range}} ดังนี้
                </div>
                <div style="margin-top:15px">
                    <table style="margin-left:100px">
                        @foreach ($auditors as $auditor)
                            @foreach ($auditor->auditors_status_many as $auditorStatus)
                                @foreach ($auditorStatus->auditors_list_many as $key => $item)
                                    <tr>
                                        <td style="width: 300px">({{$key+1}}). {{$item->temp_users}}</td>
                                        <td style="width: 300px">{{$item->temp_departments}}</td>
                                    </tr>
                                @endforeach
                            @endforeach
                        @endforeach
                ...
                <div  style="margin-left: 110px; margin-top:15px">
                    เอกสารกำหนดการตรวจประเมิน ณ สถานประกอบการ
                        @if (!is_null($trackingAuditor->FileAuditors2) &&  $trackingAuditor->FileAuditors2 != '')
                            <p style="margin-left:15px">
                                (1).  <a style="text-decoration: none" href="{{url('funtions/get-view/'.$trackingAuditor->FileAuditors2->url.'/'.( !empty($trackingAuditor->FileAuditors2->filename) ? $trackingAuditor->FileAuditors2->filename :  basename($trackingAuditor->FileAuditors2->new_filename)   ))}}" target="_blank">
                                        {!! HP::FileExtension($trackingAuditor->FileAuditors2->filename)  ?? '' !!} {{$trackingAuditor->FileAuditors2->filename}}
                                    </a>
                            </p>  
