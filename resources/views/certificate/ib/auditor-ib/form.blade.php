        ...
        <div class="col-md-9">
          <div class="form-group {{ $errors->has('reference_refno') ? 'has-error' : ''}}">
           ...
           <div class="form-group {{ $errors->has('org_name') ? 'has-error' : ''}}">
                    ...
                    <div class="col-md-7">
                     {!! Form::text('no', (!empty($tracking->name) ? $tracking->name  : null) , ['id' => 'org_name', 'class' => 'form-control no-drop', 'placeholder'=>'', 'readonly' => true]); !!}
                    </div>
           </div>
           <div class="form-group {{ $errors->has('name_unit') ? 'has-error' : ''}}">
                    {!! HTML::decode(Form::label('name_unit', 'ชื่อหน่วยตรวจสอบ'.' :', ['class' => 'col-md-5 control-label'])) !!}
                    <div class="col-md-7">
                     {!! Form::text('name_unit', (!empty($tracking->name_unit) ? $tracking->name_unit  : null) , ['id' => 'name_unit', 'class' => 'form-control', 'placeholder'=>'', 'disabled' => true]); !!}
                    </div>
           </div>
                    ...
                    <div class="col-md-7">
                        @if (isset($auditor))
                            @if ($auditor->message_record_status == 1)
                                <a class="button btn btn-warning" href="{{route('certificate.auditor-ibs.create-tracking-ib-message-record',['id' => $auditor->id])}}"><i class="fa fa-book" aria-hidden="true"> </i></a>
                                @elseif($auditor->message_record_status == 2)
                                <a class="button btn btn-info" href="{{route('certificate.auditor-ibs.view-tracking-ib-message-record',['id' => $auditor->id])}}"><i class="fa fa-book" aria-hidden="true"> </i></a>
                            @endif
