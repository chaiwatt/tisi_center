        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('confirm-signature-btn').addEventListener('click', () => {
                ...
                if (selectedId && activeSignatureBlock) {
                    ...
                    if (selectedSigner) {
                        // **NEW**: เก็บ signer id, name, และ position ไว้ใน data attribute
            ...
            function saveData(status) {
                ...
                // **NEW**: ตรวจสอบลายเซ็นก่อนบันทึก (เฉพาะ status 'final')
                if (status === 'final') {
                    let allSigned = true;
                    signatureBlocks.forEach(block => {
                ...
                // รวบรวมข้อมูลผู้ลงนาม
                signatureBlocks.forEach(block => {
                    if (block.hasAttribute('data-signer-id')) {
                        signersArray.push({
                            id: block.getAttribute('data-signer-id'),
                            name: block.getAttribute('data-signer-name') || '', // << เพิ่ม name
                ...
                fetch("{{ route('ib.save-html-template') }}", {
                    ...
                    body: JSON.stringify({ 
                        ...
                        templateType: templateType,
                        status: status,
                        signers: signersArray // << ส่งข้อมูลผู้ลงนามไปด้วย
                    })
                })
                .then(response => {
                    if (!response.ok) {
