                            ...
                            <div class="col-md-8">
                                <select name="set_standard[]" id="set_standard" class="select2-multiple" multiple data-placeholder="- เลือกมาตรฐาน -">
                                     @foreach ($tisiEstandardDraftPlans as $tisiEstandardDraftPlan)
                                        <option value="{{ $tisiEstandardDraftPlan->id }}" {{ old('set_standard') == $tisiEstandardDraftPlan->id ? 'selected' : '' }}>
                                            {{ $tisiEstandardDraftPlan->estandard_offers_to->standard_name }} ( {{ $tisiEstandardDraftPlan->estandard_offers_to->refno }}) <!-- ใช้ฟิลด์ name หรือฟิลด์ที่เหมาะสมจากโมเดล set_standard -->
                                        </option>
                                    @endforeach
                                </select>
