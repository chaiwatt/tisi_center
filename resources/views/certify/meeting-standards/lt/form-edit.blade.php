...
<div class="form-group {{ $errors->has('commitee_id') ? 'has-error' : '' }}">
 ...
 @if (!empty($meetingstandard))
   @if (!empty($meetingstandard->meeting_group))
        <div class="form-group {{ $errors->has('draft_plan_id') ? 'has-error' : '' }}" style="margin-top: 50px">
            ...
            <div class="col-md-8">
                ...
                @if (!empty($meetingGroupItems))
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach (json_decode($meetingstandard->meeting_group) as $item)
                                @php
                                    $estandardOffer = App\Models\Tis\TisiEstandardDraftPlan::find($item->id);
                                @endphp
                                <tr>
                                    <td class="text-center">{{ $loop->iteration }}</td>
                                    <td>
                                        @if($estandardOffer && $estandardOffer->estandard_offers_to)
                                            {{ $estandardOffer->estandard_offers_to->standard_name }} ({{ $estandardOffer->estandard_offers_to->refno }})
                                        @else
                                            ไม่พบข้อมูลมาตรฐาน (ID: {{ $item->id }})
                                        @endif
                                    </td>
                                    <td>
                                        {{-- เปลี่ยน name ให้ส่ง ID เป็น key ของ array --}}
                                        <select class="form-control" name="items[{{ $item->id }}][status]">
                                            <option value="1" {{ ($item->status ?? '') == '1' ? 'selected' : '' }}>อนุมัติจัดทำแผน</option>
                                            <option value="2" {{ ($item->status ?? '') == '2' ? 'selected' : '' }}>ไม่อนุมัติจัดทำแผน</option>
                                        </select>
                                    </td>
                                    <td>
                                        {{-- เปลี่ยน name ให้ส่ง ID เป็น key ของ array --}}
                                        <input type="text" class="form-control" name="items[{{ $item->id }}][note]" value="{{ $item->note ?? '' }}">
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
...
<div class="form-group {{ $errors->has('commitee_id') ? 'has-error' : ''}}">
 ...
 <div class="form-group {{ $errors->has('budget') ? 'has-error' : ''}}">
    <label for="budget" class="col-md-3 control-label">เบี้ยประชุม : <span class="text-danger">*</span></label>
    <div class="col-md-8">
        <input type="text" name="budget" id="budget" class="form-control" value="{{ $meetingstandard->budget ?? old('budget') }}" required>
        {!! $errors->first('budget', '<p class="help-block">:message</p>') !!}
    </div>
</div>

<div class=" {{ $errors->has('attach') ? 'has-error' : ''}}">
    {!! Html::decode(Form::label('attach', 'รายงานการประชุม'.' : ', ['class' => 'col-md-3 control-label'])) !!}
    <div class="col-md-9" >
...
@if ( $meetingstandard->finish == null)
