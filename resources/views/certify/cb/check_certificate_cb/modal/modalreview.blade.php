...
<div class="modal fade" id="ReviewModal" tabindex="-1" role="dialog" aria-labelledby="ReviewModalLabel" aria-hidden="true">
  ...
  @if(!is_null($review))
   {!! Form::open(['url' => 'certify/check_certificate-cb/update_review/'.$review->id,
                    ...
                    <div class="col-md-7 text-left">
                        @if (!is_null($review->FileReview1) &&  $review->FileReview1 != '')
                            <p id="deleteFlieAttach">
                                <a href="{{url('certify/check/file_cb_client/'.$review->FileReview1->file.'/'.( !empty($review->FileReview1->file_client_name) ? $review->FileReview1->file_client_name :  basename($review->FileReview1->file)))}}" target="_blank">
            ...
            <div class=" {{ $errors->has('attach') ? 'has-error' : ''}}">
                ...
                <div class="col-md-7 text-left">
                          @if (!is_null($review->FileReview2) &&  $review->FileReview2 != '')
                            <p id="deleteAttach">
                                <a href="{{url('certify/check/file_cb_client/'.$review->FileReview2->file.'/'.( !empty($review->FileReview2->file_client_name) ? $review->FileReview2->file_client_name :   basename($review->FileReview2->file) ))}}" target="_blank">
...
<script>
    ...
    jQuery(document).ready(function() {
        var review = '{{  !empty($certi_cb->review)  ? $certi_cb->review : null  }}';
         if(review == 2){
