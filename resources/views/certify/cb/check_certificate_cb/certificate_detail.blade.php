...
<div class="row">
    <div class="col-sm-12">
        <label class="col-md-3 text-right control-label">หมานเลขการรับรองที่ : </label> 
    ...
    <div class="col-sm-12">
          <label class="col-md-3 text-right control-label">หน่วยรับรอง : </label> 
          <p class="col-md-8 border-dot-bottom"> {!! !empty($export->name_standard)?  $export->name_standard:'-'  !!} </p>
   </div>
   <div class="col-sm-12">
          <label class="col-md-3 text-right control-label">สถานที่ตั้งห้องหน่วยรับรอง : </label> 
   ...
   <div class="col-sm-12">
          <label class="col-md-3 text-right control-label">สถานะของห้องหน่วยรับรอง : </label> 
          <p class="col-md-8 border-dot-bottom"> {!!  !empty($certi_cb_primary->TitleStatus->title)   ? $certi_cb_primary->TitleStatus->title : '-'  !!} </p>
   </div>
   <div class="col-sm-12">
          ...
          <p class="col-md-8">
            ...
            if(!empty($export->certificate_newfile)){
                            ...
                            $text =   '<a href="'. ( url('certify/check/file_cb_client').'/'.$export->attachs.'/'. ( !empty($export->attachs_client_name) ? $export->attachs_client_name :  basename($export->attachs)  )).'" target="_blank">
                                            '. HP::FileExtension($export->attachs).' 
                            </a> ';
           }else if(!is_null($certi_cb)){
            ...
            <div class="table-responsive repeater-file">
                ...
                <table class="table color-bordered-table info-bordered-table" id="myTable">
                    ...
                    <tbody data-repeater-list="detail">
                        @if ($cert_cbs_file_all and $cert_cbs_file_all->count() > 0)
                            @foreach ($cert_cbs_file_all as $key => $item)
                                    ...
                                    <td class="text-center">
                                        ...
                                        @php
                                            ...
                                            if(!empty($item->app_no)){
                                                $standard_change_id =   App\Models\Certify\ApplicantCB\CertiCb::where('app_no',$item->app_no)->value('standard_change');
...
@include ('certify/cb/check_certificate_cb/modal.add_attachment')
@include ('certify/cb/check_certificate_cb/modal.edit_modle')
          </div>
    </div>
</div>
                </div>
    ...
    $(document).ready(function() {
       ...
       $("body").on("click", ".del-attach", function() {
                ...
                Swal.fire({
                    ...
                    $.ajax({
                            method: "POST",
                                url: "{{ url('certify/certificate_detail-cb/del_attach') }}",
                                data: {
                                    "_token": "{{ csrf_token() }}",
                                    "id": id
                            },
