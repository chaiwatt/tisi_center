...
<div id="collapse" class="panel-collapse collapse in">
  <br>
  @foreach($cost->CertiCbHistorys as $key => $item)
            ...
            <div class="white-box" style="border: 2px solid #e5ebec;">
                ...
                @endphp              
                 ...
                 @foreach(array_slice($attachs, -1) as $scope)
                         ...
                         <p>
                             <a href="{{url('certify/check/file_cb_client/'.$scope->file.'/'.( !empty($scope->file_client_name) ? $scope->file_client_name : basename($scope->file) ))}}" target="_blank">
                         ...
                         <p> 
                             @if(isset($files->attach_files))
                               {{  @$files->file_desc_text  }}
                                    <a href="{{url('certify/check/file_cb_client/'.$files->attach_files.'/'.( !empty($files->file_client_name) ? $files->file_client_name :   basename($files->attach_files) ))}}" target="_blank">
