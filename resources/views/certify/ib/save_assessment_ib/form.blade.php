...
<div class="modal fade" id="modal-email-to-expert">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            @foreach ($certiIBAuditorsLists as $certiIBAuditorsList)
                    @if ($certiIBAuditorsList->auditorInformation->email != null)
                        @php
                                    ...
                                    @if ($certiIBAuditorsList->auditorInformation->email != null)
                                        <td>
                                            <input type="checkbox" class="expert-email-checkbox" 
                                                checked 
                                                data-email="{{$certiIBAuditorsList->auditorInformation->email}}">
                                        </td>
                                        <td>
                                            {{$certiIBAuditorsList->auditorInformation->title_th}}{{$certiIBAuditorsList->auditorInformation->fname_th}} {{$certiIBAuditorsList->auditorInformation->lname_th}}
                                        </td>
                                        <td>
                                            {{$certiIBAuditorsList->auditorInformation->email}}
                                        </td>
                                        <td></td>
                                        @else
                                        <td></td>
                ...
                <div class="row">
                   ...
                   <div class="col-md-12 mt-2">
                    ...
                    <p>
                        @php
                            $config = HP::getConfig();
                            $url = $config->url_center.'create-by-ib-expert/' . $assessment->id .'?token='.$assessment->expert_token;
                        @endphp
                        {{-- {{$assessment}} --}}
    ...
    <div class="col-md-12">
        ...
        <div class="form-group">
            ...
            <div class="col-md-6">
                ...
                <div class="col-md-8">
                    {{-- @if(isset($app_no))
                        <select name="auditors_id" 
                                class="form-control" 
                                id="auditors_id" 
                                required>
                            <option value="">- เลขคำขอ -</option>
                            @foreach($app_no as $key => $value)
                                <option value="{{ $key }}" {{ old('auditors_id') == $key ? 'selected' : '' }}>
                                    {{ $value }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <input type="hidden" 
                               class="form-control" 
                               value="{{ $assessment->app_certi_ib_id ?? null }}" 
                               name="app_certi_ib_id" 
                               id="app_certi_ib_id">
                    @else 
                        <input type="text" 
                               class="form-control" 
                               value="{{ $assessment->AuditorsTitle ?? null }}" 
                               disabled>
                    @endif --}}
    ...
    <div class="col-sm-12 m-t-15 "  id="box-required">
        <table class="table color-bordered-table primary-bordered-table">
            ...
            <tr>
                ...
                <th class="text-center" width="10%">มอก. 17020  : ข้อ</th>
                    ...
                    <tr>
                        ...
                        <td style="padding: 0px;">
                            <input type="hidden" 
                                ...
                                class="form-control">
                            <textarea name="detail[report][]" 
                                    class="form-control input_required auto-expand" 
                                    rows="5" style="border-right: 1px solid #ccc;" required>{{ $item->report ?? '' }}</textarea>
                        </td>
                        <td style="padding: 0px;">
                            <textarea name="detail[notice][]" 
                                    class="form-control input_required auto-expand" 
                                    rows="5" style="border-left: none; border-right: 1px solid #ccc;" required>{{ $item->remark ?? '' }}</textarea>
                        </td>
                        <td style="padding: 0px;">
                            <textarea name="detail[no][]" 
                                    class="form-control input_required auto-expand" 
                                    rows="5" style="border-left: none; border-right: 1px solid #ccc;" required>{{ $item->no ?? '' }}</textarea>
                        </td>
                        <td style="padding-left: 15px;vertical-align:top">
                            <select name="detail[type][]" 
                                    class="form-control type input_required select2" 
                                    required>
                                <option value="">-เลือกประเภท-</option>
                                <option value="1" {{ ($item->type ?? null) == '1' ? 'selected' : '' }}>ข้อบกพร่อง</option>
                                <option value="2" {{ ($item->type ?? null) == '2' ? 'selected' : '' }}>ข้อสังเกต</option>
                            </select>
                        </td>
                        <td class="text-center div_hide">
...
{{-- @else 
<div class="clearfix"></div>
   <a  href="{{ url("certify/save_assessment-ib") }}"  class="btn btn-default btn-lg btn-block">
                    ...
                    $.ajax({
                        url:'{{ url('certify/save_assessment-ib/certi_ib') }}/' + $(this).val()
                            ...
                            if(object.certi_ib != '-'){ 
                                ...
                                $('#laboratory_name').val(certi_ib.name_unit); 
                                $('#app_certi_ib_id').val(certi_ib.id); 
                            }else{
                                $('#applicant_name').val(''); 
                                $('#laboratory_name').val(''); 
                                $('#app_certi_ib_id').val(''); 
    ...
    $('#button_email_to_expert').on('click', function () 
        {
                ...
                $.ajax({
                    url: "{{route('save_assessment.email_to_ib_expert')}}",
                    method: "POST",
                    data: {
                        _token: _token,
