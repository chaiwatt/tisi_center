...
<div class="row">
   ...
   {{$selectedCertiLab->id}} --}}
            ...
            <input type="hidden" name="signaturePositionsJson" id="signaturePositionsJson">
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered color-table primary-table" id="myTable" width="100%">
                                                ...
                                                <tr>
                                                    ...
                                                    <th class="text-center" width="20%">ชื่อผู้ตรวจประเมิน</th>
                                                    {{--  --}}
...
<script src="https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.min.js"></script>
    ...
    //     ];
        ...
        function  submit_form(){
            let data = {};
            // ดึง input ที่มี name เริ่มต้นด้วย 'group'
            $("input[name^='group']").each(function() {
                    // ดึง name ของ input เช่น "group[0][temp_users][]"
                    let name = $(this).attr("name");

                    // ใช้ regex เพื่อแปลง name ให้เป็น path ของ array
                    let keys = name.match(/\[(.*?)\]/g).map(k => k.replace(/\[|\]/g, ''));
                    ...
                    temp[lastKey].push($(this).val());
            });

    // console.log(data); // ดูค่าที่ดึงได้
        ...
        function statusChange(that) {
            ...
            // let inputCategory = $('<input type="text" class="form-control item" data-name="category" value="" readonly>');
            // inputCategory.appendTo(tdDepartments);
                ...
                $.ajax({
                    ...
                    success: function (resp) {
                        ...
                        auditors.forEach(auditor => {
                            ...
                            let td2 = $('<td style="text-align:center">');
        ...
        function modalHiding(that) {
            ...
            let tdUsers = $(that).closest('.repeater-item').find('.td-users');
            let tdDepartments = $(that).closest('.repeater-item').find('.td-departments');
            let tdStatus = $(that).closest('.repeater-item').find('select.status');
            let empty = true;
            let groupVal = "";
            tdUsers.children().remove();
            tdDepartments.children().remove();

            let selectedText = tdStatus.find(':selected').text(); // ข้อความที่แสดงใน select
            let selectedValue = tdStatus.val(); // ค่า value ของ select
            ...
            checkboxes.each(function () {
                if ($(this).is(':checked')) {
                    ...
                    let inputCategory = $('<input type="hidden" class="form-control item" data-name="category" value="'+selectedText+'" readonly>');
                    inputCategory.appendTo(tdDepartments);
            ...
            if (empty) {
                ...
                let inputCategory = $('<input type="hidden" class="form-control item" data-name="category" value="" readonly>');
                inputCategory.appendTo(tdDepartments);
            }
        ...
        function setRepeaterIndex() {
            ...
            $('#table-body').find('tr.repeater-item').each(function () {
                ...
                // เพิ่ม input field สำหรับ category ที่มี default เป็น 1
                // if ($(this).find('input[data-name="category"]').length === 0) {
                //     let inputCategory = $('<input type="hidden" class="form-control item" data-name="category" value="1">');
                //     inputCategory.appendTo($(this));
                // }
